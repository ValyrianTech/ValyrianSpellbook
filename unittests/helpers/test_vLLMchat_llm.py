...
class TestVLLMchatLLMThinkingLevel(unittest.TestCase):
    """Test thinking_level handling in VLLMchatLLM"""
    ...
    def test_get_completion_text_with_thinking_level(self, mock_log, mock_sender, mock_channel, mock_broadcast, mock_openai, mock_ws):
        """Test completion with thinking_level - covering line 35"""
        from helpers.vLLMchat_llm import VLLMchatLLM
        ...
        self.assertEqual(result, 'Hello!')
        mock_log.info.assert_any_call('Thinking level: high -> Ignored (vLLM chat API does not support thinking levels)')

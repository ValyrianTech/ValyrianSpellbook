...
def sign_message(address, message, private_key):
    ...
    return SignMessage(key=key, message=BitcoinMessage(message))
...
# Todo : change all uses of these function so we can remove this file completely (because old code doesn't work on python3)

...
def parse_derivation_path(derivation_path):
    ...
    if re.match(BIP32_DERIVATION_PATH_REGEX, derivation_path) is None:
        raise ValueError(f'Derivation path is invalid: {derivation_path}')
...
def get_xpub_child(xpub, child_index):
    ...
    if child_index >= HARDENED:
        raise ValueError('Only non-hardened child xpubs keys can be derived from an xpub key')
...
def raw_bip32_ckd(rawtuple, i):
    ...
    vbytes, depth, fingerprint, _oldi, chaincode, key = rawtuple
    i = int(i)

...
def get_addresses_from_xpub(xpub, i=100):
    ...
    for index in range(i):
        public_key = bip32_ckd(pub0, index)
        hex_key = encode_pubkey(bip32_extract_key(public_key), 'hex_compressed')
...
def get_xpriv_keys(mnemonic, passphrase="", i=1):
    ...
    xprivs = []
    for index in range(i):
        derived_private_key = bip32_ckd(bip32_ckd(bip32_ckd(private_key, 44+HARDENED), HARDENED+COIN_TYPE), HARDENED+index)
...
def get_xpub_keys(mnemonic, passphrase="", i=1):
    ...
    xpubs = []
    for index in range(i):
        derived_private_key = bip32_ckd(bip32_ckd(bip32_ckd(priv, 44+HARDENED), HARDENED+COIN_TYPE), HARDENED+index)

...
6. Set up a cron that executes **spellbook.py check_triggers** (for example every 10 minutes) depending on the needs of your app, alternatively if you have a bitcoin node you could use the blocknotify option to run **spellbook.py check_triggers** each time a new block is found.

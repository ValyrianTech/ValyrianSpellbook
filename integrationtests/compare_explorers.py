...
class Comparison:
    def __init__(self, *args):
        ...
        print('\n\n#####################################################################################################')
        print(f'# Comparing: {self.spellbook_args}')
        print('#####################################################################################################')
        ...
        if args[0] == 'get_transaction':
            ...
            if not all(n_txs[0] == item for item in n_txs):
                ...
                for explorer in self.responses:
                    print('{}: {}'.format(explorer, len(self.responses[explorer])[0]['transactions']))
            ...
            if not all(n_txs[0] == item for item in n_txs):
                ...
                for explorer in self.responses:
                    print('{}: {}'.format(explorer, len(self.responses[explorer])[0]['utxos']))
    ...
    def get_response_from_explorer(self, explorer):
        ...
        args_with_explorer = self.spellbook_args + [f'-e={explorer}']
    ...
    def compare_response_data(self):
        ...
        for i in range(len(EXPLORERS)):
            print(f'\nComparing {EXPLORERS[i]} and {EXPLORERS[i-1]}: .... ',)
    ...
    def compare_dict_items(self, dicts, item_names):
        ...
        for item_name in item_names:
            ...
            if not all(items[0] == item for item in items):
                all_ok = False
                for explorer in self.responses:
                    print(f'{explorer} -> {item_name}: {dicts[explorer][item_name]}')
